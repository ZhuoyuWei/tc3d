...
PRED_OUT=$OUTPUT_DIR/res_${ntree_limit}
mkdir $PRED_OUT
...
python model_nn.py predict-all ${MODEL_FILE} ${PRED_DATA} ${PRED_OUT} ${ntree_limit} >> $OUTPUT_DIR/log_${ntree_limit}
python score.py score-all  ${PRED_OUT} ${PRED_ANS} >> $OUTPUT_DIR/log_${ntree_limit}